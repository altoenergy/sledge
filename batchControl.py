...
while (True):
    ...
    try:
        if (action == "?"):
            ...
            print "remote    :  set remote to true/false"
            print "debug     :  set debug to true/false"
            print "batch     :  set batch to which operations apply"
            print "package   :  package portfolio with parameters"
            ...
            print "export    :  export a cache item to the clipboard"
            print "clear     :  clear a cache item"
            print "quit      :  quit"
            print "?         :  display help"
        elif (action == "debug"):
            debug = util.get_bool_input("debug (%s) : " % debug, debug)
            logging.getLogger().setLevel(level = logging.DEBUG if debug else logging.INFO)
        elif (action == "remote"):
            remote = util.get_bool_input("remote (%s) : " % remote, remote)
            ...
            batch = util.get_str_input("batch (%s) : " % batch, batch)
        elif (action == "package"):
            ...
            key = util.get_str_input("key () : ", "")
            path = "%s/%s" % (batch, key)
            if (remote):
                k = cloud.call(cache.delete_prefix, "%s" % path)
                logging.info(k)
            else:
                cache.clear(path, False)
        elif (action == "quit"):
            sys.exit()
        else:
